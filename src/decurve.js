...
const decurve = (shape, accuracy = 1) => {
  ...
  s.map((point, i) => {
    if (point.curve) {
      ...
      straighten(prevPoint, point, accuracy, 200)
        .map(p => d.push(p))
    } else {
      d.push(point)
    }
...
const straighten = (prevPoint, point, accuracy, maxStack) => {
  ...
  return [
    ...straighten(prevPoint, midPoint, accuracy, --maxStack),
    ...straighten(midPoint, lastPoint, accuracy, --maxStack)
  ]
}

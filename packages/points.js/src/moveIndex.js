...
const countLinePoints = (lines) =>
  lines.reduce((count, points) => count + countPoints(points), 0);
...
const joinLines = (lines) =>
  lines.reduce((shape, line) => [...shape, ...line], []);
...
const reorderLines = (lines, offset) =>
  [...lines].splice(offset).concat([...lines].splice(0, offset));
...
const reorderPoints = (points, offset) => {
  ...
  if (isJoined(points)) {
    return [...nextPoints, ...[...points].splice(1, offset)];
  }

  return [...nextPoints, ...[...points].splice(0, offset + 1)];
};

const splitLines = (shape) =>
  shape.reduce((lines, point) => {
    if (point.moveTo) {
      lines.push([]);
    }
